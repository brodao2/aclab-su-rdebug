...
export class ToggleTreeItem extends RdbgTreeItem {
	...
	async enable(session: vscode.DebugSession | undefined) {
		...
		const maxLogSize = vscode.workspace.getConfiguration(SU_RDEBUG).get<number>("maxTraceLogSize") || 50000;
		if (this.config.recordAndReplay) {
			...
			if (this.config.traceCall) {
				...
				const traceReturn = vscode.workspace.getConfiguration(SU_RDEBUG).get<boolean>("enableTraceReturnValue");
				if (traceReturn) {
				...
				const traceParams = vscode.workspace.getConfiguration(SU_RDEBUG).get<boolean>("enableTraceParameters");
				if (traceParams) {
